# compare initialization value with MNIST dataset
...
if __name__ == '__main__':
    ...
    for key, weight_type in weight_init.items():
        nets[key] = (MultiLayeredNet(input_size=784, hidden_size_list=[100, 100, 100, 100], output_size=10, weight_init_std=weight_type))
        train_loss[key] = []
        train_acc[key] = []
        test_acc[key] = []
    ...
    for i in range(iters_num):
        ...
        print('Elapsed time[s]: %.3f, current iter: %d, current %s loss: %.4f'
         % (time.time() - start_time, i, weight_init[key], loss), end='\r')

    plt.figure(1)
    plt.plot(np.arange(0, iters_num / 5), train_loss['std=0.01'][0:2000], '--Dr', label=weight_init['std=0.01'], linewidth=2)
    plt.plot(np.arange(0, iters_num / 5), train_loss['Xavier'][0:2000], '--Dg', label=weight_init['Xavier'], linewidth=2)
    plt.plot(np.arange(0, iters_num / 5), train_loss['He'][0:2000], '--Db', label=weight_init['He'], linewidth=2)
    plt.xlabel('iterations')
    plt.ylabel('loss')
    plt.title('Comparison of learning upon initialization values of weight')
    plt.legend()
    plt.grid()
